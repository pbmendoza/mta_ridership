"""Shared monthly-download infrastructure for MTA data scripts.

Provides the complete download pipeline (pagination, completeness checks,
duplicate verification, concurrent execution, rich progress UI) so that
individual update scripts only need to supply domain-specific constants.
...
Usage::
    ...
    config = DatasetConfig(
        ...
        soda_order_clause="transit_timestamp ASC, station_complex_id ASC, ...",
        column_order=["transit_timestamp", "transit_mode", ...],
        example_year="2025",
    )
    sys.exit(run_download_pipeline(config))
"""
...
class DatasetConfig:
    """Domain-specific constants that differ between ridership and turnstile."""

    description: str
    config_filename: str
    output_subdir: str
    timestamp_field: str
    soda_order_clause: str
...
def get_month_where_clause(config: DatasetConfig, year: int, month: int) -> str:
    """Build SoQL where clause for one month."""
    start_ts, end_ts = compute_date_range(year, month)
    ts = config.timestamp_field
    return f"{ts} >= '{start_ts}' AND {ts} < '{end_ts}'"
...
def check_first_day_has_data(
    ...
    where_clause = f"{ts} >= '{first_day_start}' AND {ts} <= '{first_day_end}'"
...
def check_last_day_has_data(
    ...
    where_clause = f"{ts} >= '{last_day_start}' AND {ts} <= '{last_day_end}'"
    count = count_rows(session, endpoint, headers, where_clause)
    return count > 0
...
def count_unique_days(
    ...
    ts = config.timestamp_field
    start_ts, end_ts = compute_date_range(year, month)
    where_clause = f"{ts} >= '{start_ts}' AND {ts} < '{end_ts}'"
    params = {
        "$select": f"date_trunc_ymd({ts}) AS day",
        "$where": where_clause,
        "$group": "day",
    }
    rows = request_json(session, endpoint, params, headers)
...
def merge_header(column_order: List[str], seen_rows: List[Dict[str, str]]) -> List[str]:
...
def download_month(
    ...
    with output_path.open("w", encoding="utf-8", newline="") as handle:
        ...
        while True:
            params = {
                "$where": where_clause,
                "$order": config.soda_order_clause,
                "$limit": str(page_size),
                "$offset": str(offset),
            }
            rows = request_json(session, endpoint, params, headers)
            ...
            if not header:
                header = merge_header(config.column_order, rows)
                writer = csv.DictWriter(handle, fieldnames=header)
                writer.writeheader()

            for row in rows:
                if config.timestamp_field not in row:
                    continue
...
def process_task(
    ...
    where_clause = get_month_where_clause(config, year, month)
    ...
    try:
        ...
        has_first_day_data = check_first_day_has_data(session, task.endpoint, headers, config, year, month)
        has_last_day_data = check_last_day_has_data(session, task.endpoint, headers, config, year, month)
        expected_days = get_last_day_of_month(year, month)
        unique_days = count_unique_days(session, task.endpoint, headers, config, year, month)
...
def build_arg_parser(config: DatasetConfig) -> argparse.ArgumentParser:
    """Build the CLI argument parser for a download script."""
    parser = argparse.ArgumentParser(description=config.description)
    parser.add_argument(
        "--year",
        type=int,
        default=None,
        help=f"Year to download (e.g., {config.example_year}). If omitted, downloads all years in config.",
    )
    parser.add_argument(
        "--month",
        type=int,
...
def run_download_pipeline(config: DatasetConfig) -> int:
    """Run the full download pipeline for the given dataset config.
